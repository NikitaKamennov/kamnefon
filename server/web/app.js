...
async function loadTranslations(lang) {
  try {
    ...
    const response = await fetch(
      `${API_BASE}/translations/${lang}?v=${timestamp}`
    );
    if (!response.ok) {
    ...
    console.log(
      "[I18N] API translations loaded, keys:",
      Object.keys(apiTranslations).length,
      "sample keys:",
      Object.keys(apiTranslations).slice(0, 10)
    );
    ...
    console.log(
      "[I18N] Translations loaded successfully, total keys:",
...
async function loadTURNConfig() {
  try {
    ...
    if (!response.ok) {
      ...
      console.error(
        "[TURN] Failed to load TURN config, status:",
        response.status,
        errorText
      );
    ...
    if (data.iceServers && Array.isArray(data.iceServers)) {
      ...
      console.error(
        "[TURN] Invalid config format - missing iceServers array. Data:",
        data
      );
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  const savedLang =
    localStorage.getItem("preferredLanguage") || detectLanguage();
  try {
    await loadTranslations(savedLang);
  } catch (error) {
    console.error(
      "[I18N] Failed to load translations, retrying with English..."
    );
    // Retry with English as fallback
    if (savedLang !== "en") {
      try {
...
function setupEventListeners() {
  ...
  const closeContactCardBtn = document.getElementById("close-contact-card-btn");
  ...
  document
    .getElementById("accept-invite-btn")
...
async function handleLogin(e) {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (!statusData.registration_enabled) {
        showError(
          errorEl,
          statusData.message ||
            "Get an invite from family organizer to use Family Callbook"
        );
        hideLoading();
        return;
      }
    ...
    const addContactBtn = document.getElementById("add-contact-btn");
    if (addContactBtn) {
      addContactBtn.style.display = currentUser.is_first_user
        ? "block"
        : "none";
    }

    // Show/hide backup/restore section based on first user status
    const backupRestoreSection = document.getElementById(
      "backup-restore-section"
    );
    if (backupRestoreSection) {
      backupRestoreSection.style.display = currentUser.is_first_user
        ? "block"
        : "none";
    }
...
function displayPendingInvites(pendingInvites) {
  ...
  pendingInvites.forEach((invite) => {
    ...
    const escapedInviteLinkForOnclick = inviteLink
      .replace(/'/g, "\\'")
      .replace(/"/g, "&quot;");
    ...
    item.innerHTML = `
            ...
            <div class="invite-actions">
                ...
                ${
                  navigator.share
                    ? `<button class="btn-icon" onclick="shareInviteLink('${escapedInviteLinkForOnclick}')" title="Share">📤</button>`
                    : ""
                }
  ...
  if (contactsList && contactsList.parentNode) {
    contactsList.parentNode.insertBefore(
      pendingSection,
      contactsList.nextSibling
    );
...
function displayContacts(contacts) {
  ...
  contacts.forEach((contact) => {
    ...
    const pushIcon = hasPush
      ? '<span class="push-indicator" title="Push notifications enabled">🔔</span>'
      : "";
    ...
    item.innerHTML = `
            ...
            <div class="contact-actions">
                ...
                ${pushIcon}
                ${
                  currentUser && currentUser.is_first_user
                    ? `<button class="btn-icon" onclick="renameUser('${contactUserId}', '${contactName}')" title="Rename">✏️</button>`
                    : ""
                }
                ...
                ${
                  currentUser && currentUser.is_first_user
                    ? `<button class="btn-icon" onclick="deleteContact('${contactUserId}', '${contactName}')" title="Delete Contact">🗑️</button>`
                    : ""
                }
            </div>
        `;
    listEl.appendChild(item);
    const infoEl = item.querySelector(".contact-info");
    if (infoEl) {
      infoEl.addEventListener("click", (e) => {
        // если кликнули по действиям — карточку не открываем
...
// Открыть карточку по ID (утилита)
function openContactCardById(contactUserId) {
  const contact = contactsData.find(
    (c) => (c.id || c.contact_id) === contactUserId
  );
  if (contact) {
    openContactCard(contact);
  }
...
function openContactCard(contact) {
  ...
  const statusEl = document.getElementById("contact-card-status");
  if (statusEl) {
  ...
  // Индикатор push
  const pushEl = document.getElementById("contact-card-push");
  if (pushEl) {
    const hasPush = !!contact.has_push;
    pushEl.textContent = hasPush ? "🔔 Push" : "🔕 Push";
    pushEl.style.opacity = hasPush ? "1" : "0.6";
  }

  // Сохраним данные в dataset, чтобы не таскать их в замыканиях
  modal.dataset.contactId = contactId;
  modal.dataset.contactName = contactName;

  // Кнопки действий
  const resendBtn = document.getElementById("contact-card-resend-btn");
  if (resendBtn) {
    resendBtn.onclick = () => resendInvite(contactId);
  }

  const callAudioBtn = document.getElementById("contact-card-call-audio-btn");
  if (callAudioBtn) {
    callAudioBtn.onclick = () =>
      initiateCall(contactId, contactId, "audio", contactName);
  }

  const callVideoBtn = document.getElementById("contact-card-call-video-btn");
  if (callVideoBtn) {
    callVideoBtn.onclick = () =>
      initiateCall(contactId, contactId, "video", contactName);
  }

  const renameBtn = document.getElementById("contact-card-rename-btn");
  const deleteBtn = document.getElementById("contact-card-delete-btn");

  // Права организатора
  const isOrganizer = currentUser && currentUser.is_first_user;

  if (renameBtn) {
    renameBtn.style.display = isOrganizer ? "inline-flex" : "none";
    renameBtn.onclick = isOrganizer
      ? () => renameUser(contactId, contactName)
      : null;
  }

  if (deleteBtn) {
    deleteBtn.style.display = isOrganizer ? "inline-flex" : "none";
    deleteBtn.onclick = isOrganizer
...
function closeContactCard() {
  ...
  delete modal.dataset.contactName;
}

// Обновить открытую карточку, если она открыта по этому контакту
function updateOpenContactCardUI(contactUserId) {
  if (!openContactId || openContactId !== contactUserId) return;
...
function updateContactOnlineStatus(contactUserId, isOnline) {
  console.log(
    "[STATUS] Updating contact status:",
    contactUserId,
    "isOnline:",
    isOnline
  );
  console.log(
    "[STATUS] Looking in contactsData:",
    contactsData.length,
    "contacts"
  );
  ...
  if (contact) {
    ...
    updateOpenContactCardUI(contactUserId);
    console.log("[STATUS] Updated contact data for:", contact.contact_name);
  } else {
    console.warn(
      "[STATUS] Contact not found in contactsData for user ID:",
      contactUserId
    );
    console.warn(
      "[STATUS] Available contact IDs:",
      contactsData.map((c) => c.id || c.contact_id)
    );
  }

  // Update the UI
  const contactItem = document.querySelector(
    `.contact-item[data-contact-id="${contactUserId}"]`
  );
  if (contactItem) {
    ...
    console.warn(
      "[STATUS] Contact item not found in DOM for user ID:",
      contactUserId
    );
    console.warn(
      "[STATUS] Available data-contact-id attributes:",
      Array.from(document.querySelectorAll(".contact-item")).map((el) =>
...
async function renameUser(userId, currentName) {
  ...
  try {
    ...
    if (userId === currentUser.id) {
      ...
      const mobileUsernameEl = document.getElementById(
        "mobile-current-username"
      );
      if (mobileUsernameEl) mobileUsernameEl.textContent = currentUser.username;
    }
  ...
  try {
    ...
    if (
      ...
      if (
        ...
        console.error(
          "[TURN] Failed to load TURN config, proceeding with empty config"
        );
      }
    }
    ...
    console.log(
      "[TURN] ICE servers count:",
      rtcConfig.iceServers ? rtcConfig.iceServers.length : 0
    );
      ...
      try {
        ...
        setTimeout(async () => {
          try {
            ...
            console.error(
              "Error playing remote stream after retry:",
              retryError
            );
          }
        }, 100);
    ...
    case "call-accept":
      ...
      if (currentCall && currentCall.contactId === message.from) {
        ...
        if (currentCall.waitingForConnection) {
          ...
          startCall(currentCall.contactId, currentCall.callType).catch(
            (error) => {
...
async function handleIncomingCall(message) {
  ...
  if (nameEl)
    nameEl.textContent = incomingCall.caller_username || "Incoming call";
  if (typeEl)
    typeEl.textContent =
...
async function handleOffer(message) {
  try {
    ...
    if (!currentCall) {
      console.log(
        "[OFFER] No currentCall set, creating from offer (incoming call)"
      );
      currentCall = {
    ...
    try {
      ...
      setTimeout(async () => {
        try {
          ...
          console.error(
            "[OFFER] Error playing local video after retry:",
            retryError
          );
        }
      }, 100);
    ...
    peerConnection.onconnectionstatechange = () => {
      ...
      if (state === "failed") {
        ...
        console.warn(
          "[OFFER] Connection disconnected, waiting for reconnection..."
        );
        ...
        setTimeout(() => {
          if (
            ...
            console.error(
              "[OFFER] Still disconnected after timeout, ending call"
            );
            endCall();
          }
        }, 5000);
        ...
        if (remoteVideo.srcObject) {
          remoteVideo.play().catch((err) => {
            console.error(
              "[OFFER] Error playing remote stream on connect:",
              err
            );
          });
        }
    ...
    peerConnection.oniceconnectionstatechange = () => {
      ...
      if (peerConnection.iceConnectionState === "failed") {
        ...
        if (remoteVideo.srcObject) {
          remoteVideo.play().catch((err) => {
            console.error(
              "[OFFER] Error playing remote stream on ICE connect:",
              err
            );
          });
        }
...
async function handleAnswer(message) {
  ...
  try {
    if (peerConnection.remoteDescription) {
      ...
      console.log(
        "[ANSWER] Remote description already set, ignoring duplicate answer"
      );
    ...
    console.error(
      "[ANSWER] Error setting remote description from answer:",
      error
    );
...
async function processPendingIceCandidates() {
  ...
  console.log(
    `[ICE] Processing ${pendingIceCandidates.length} pending ICE candidates`
  );
  ...
  while (pendingIceCandidates.length > 0) {
    ...
    try {
      ...
      console.log(
        "[ICE] Adding pending ICE candidate:",
...
function makeLocalVideoDraggable() {
  ...
  if (!localVideo.style.left && !localVideo.style.right) {
    ...
    setPosition(
      containerRect.width - rect.width - 20,
      containerRect.height - rect.height - 100
    );
  }
...
async function enumerateAudioOutputDevices() {
  try {
    ...
    console.log(
      "[AUDIO] Available audio output devices:",
      audioOutputDevices.length
    );
    audioOutputDevices.forEach((device, index) => {
      console.log(
        `[AUDIO] Device ${index}: id=${device.deviceId}, label="${device.label}"`
      );
    });
    ...
    if (
      ...
      audioOutputDevices.every((d) => !d.label)
    ) {
      console.log(
        "[AUDIO] No devices with labels found, retrying after delay..."
      );
      setTimeout(async () => {
        try {
          const retryDevices = await navigator.mediaDevices.enumerateDevices();
          const retryAudioDevices = retryDevices.filter(
            (device) => device.kind === "audiooutput"
          );
          if (
            retryAudioDevices.length > 0 &&
            retryAudioDevices.some((d) => d.label)
          ) {
            audioOutputDevices = retryAudioDevices;
            console.log(
              "[AUDIO] Retry successful, found devices:",
              audioOutputDevices
            );
          }
        } catch (retryError) {
...
function getEarpieceDevice() {
  return audioOutputDevices.find(
    (device) =>
...
function populateAudioOutputMenu() {
  ...
  defaultOption.innerHTML = `
        ...
        <span class="audio-output-label">${
          t("audio_speaker") || "Speaker"
        } (Default)</span>
    `;
...
function updateAudioOutputButton() {
  ...
  if (currentAudioOutputId === "default") {
    ...
    const selectedDevice = audioOutputDevices.find(
      (d) => d.deviceId === currentAudioOutputId
    );
    let icon = "🔊";
...
async function subscribeToPush() {
  ...
  try {
    ...
    try {
      ...
      console.warn(
        "Error checking/unsubscribing from existing subscription:",
        error
      );
    }
    ...
    try {
      ...
      if (subscription.getKey) {
        ...
        console.error(
          "Subscription object does not have getKey() or keys property"
        );
      ...
      if (
        !(p256dhArray instanceof Uint8Array) ||
        !(authArray instanceof Uint8Array)
      ) {
...
function uint8ArrayToBase64(uint8Array) {
  ...
  if (!(uint8Array instanceof Uint8Array)) {
    console.error(
      "uint8ArrayToBase64: Expected Uint8Array, got:",
      typeof uint8Array,
      uint8Array
    );
...
async function checkPWAInstallability() {
  ...
  console.log(
    "Note: Chrome requires user engagement (user interaction) before showing install prompt"
  );
  console.log(
    "The install icon appears in the address bar after visiting the site"
  );
}
...
async function acceptInvite() {
  ...
  try {
    ...
    const acceptResponse = await fetch(
      `${API_BASE}/invite/${pendingInvite}/accept`,
      {
        method: "POST",
        headers: {
          Authorization: `Bearer ${authToken}`,
